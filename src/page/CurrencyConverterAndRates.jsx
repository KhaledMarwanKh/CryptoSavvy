...
import { ArrowRightLeft, AlertTriangle, Table2 } from "lucide-react";
...
function CurrencyConverterAndRates() {
  ...
  const startRate = useMemo(
    () => (timeseries?.length ? timeseries[0]?.rate : null),
    [timeseries],
  );

  const changePct = useMemo(() => {
    if (startRate == null || currentRate == null || startRate === 0)
      return null;
  ...
  const fetchLatestTable = async () => {
    ...
    try {
      ...
      const url = `https://v6.exchangerate-api.com/v6/a4557ca5f83a3bf49f7fc6ef/latest/${encodeURIComponent(base)}`;
  ...
  const covert = async () => {
    ...
    try {
      ...
      const rows = Object.keys(response.history)
        .sort()
        .map((date) => ({
      ...
      if (e?.name !== "AxiosError") {
        setError(
          e?.response?.data?.message || t("currency.errors.chartFailed"),
        );
        setTimeseries([]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 px-4 py-8 text-slate-100">
      <div className="flex items-center justify-center gap-5 mb-6">
        {["Global Services", "SYP Services"].map((mode) => (
          <button
            ...
            className={`px-3 py-2 rounded-lg ${mode === currentMode ? "bg-blue-600 font-semibold" : "bg-slate-700"} transition-all duration-500`}
          >
            {mode === "Global Services"
              ? t("currency.modes.globalServices")
              : t("currency.modes.sypServices")}
          </button>
        ))}
      </div>
          ...
          <div className="mx-auto w-full max-w-6xl">
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <div className="mt-1 flex flex-wrap items-center gap-2 text-sm">
                        <span className="text-slate-400">
                          {t("currency.chart.current")}:
                        </span>
                        <span className="font-semibold">
                          {currentRate == null
                            ? "—"
                            : formatCompactNumber(currentRate?.toFixed(2))}
                        </span>
                        <span className="text-slate-400">·</span>
                        <span className="text-slate-400">
                          {t("currency.chart.change")}:
                        </span>
                        {changePct == null ? (
                          <span className="text-slate-400">—</span>
                        ) : (
                    ...
                    {loadingChart ? (
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                          ...
                          <Tooltip
                            content={
                              <CustomTooltip base={base} target={target} />
                            }
                          />
                          <Area
                            type="monotone"
                    ...
                    <div className="mt-4 space-y-3">
                      <label className="block">
                        <div className="mb-2 text-sm font-medium text-slate-300">
                          {t("currency.converter.amount")} ({base})
                        </div>
                      ...
                      >
                        ...
                        >
                          {converted == null
                            ? "—"
                            : `${formatCompactNumber(converted)} ${target}`}
                        </div>
                        <div className="mt-2 text-xs text-slate-400">
                          {t("currency.converter.rate")}:{" "}
                          <span dir="ltr" className="text-slate-200">
                            1 {base} ={" "}
                            {latestTargetRate == null
                              ? "—"
                              : formatCompactNumber(
                                  latestTargetRate?.toFixed(2),
                                )}{" "}
                            {target}
                          </span>
                        </div>
            ...
            >
              <div className="mb-4 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                ...
                <div className="text-sm text-slate-400">
                  {loadingTable
                    ? t("currency.table.loading")
                    : `${latestRates.length} ${t("currency.table.currenciesCount")}`}
                </div>
              </div>
                ...
                {latestRates.slice(0, 12).map((r) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="text-sm text-slate-400">
                          {t("currency.mobileCard.currency")}
                        </div>
                        <div className="text-lg font-semibold">
                          {r.currency}
                        </div>
                      ...
                      <thead className="sticky top-0 bg-slate-900/80 backdrop-blur-xl">
                        <tr className="text-left text-sm text-slate-300">
                          <th className="px-4 py-3 text-center font-medium">
                            {t("currency.table.columns.currency")}
                          </th>
                          <th className="px-4 py-3 text-center font-medium">
                            {t("currency.table.columns.rate")}
                          </th>
                          <th className="px-4 py-3 text-center font-medium">
                            {t("currency.table.columns.meaning")}
                          </th>
                        </tr>
                      </thead>
                        ...
                        {latestRates.map((r) => (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-center text-sm text-slate-400">
                              <span dir="ltr">
                                {" "}
                                1 {base} = {formatCompactNumber(r.rate)}{" "}
                                {r.currency}
                              </span>
                            </td>
                          </tr>
      ...
      {currentMode === "SYP Services" && <SypCurrency />}
    </div>
  );
}
