...
function extractSymbolFromText(text = "") {
  ...
  const compactPair = text
    .toUpperCase()
    .match(/\b([A-Z]{2,10})(USDT|BUSD|BTC|ETH)\b/);
...
function shouldShowMarketChart(text = "") {
  ...
  const priceIntent =
    /(price|chart|analy[sz]e|analysis|trend|support|resistance|ohlc|candles?|technical|market overview|market today)/i.test(
      text,
    ) ||
    /(سعر|الأسعار|مخطط|رسم بياني|تحليل|حلل|اتجاه|ترند|دعم|مقاومة|شموع|الشموع|تقني|تحليل فني|نظرة عامة على السوق|السوق اليوم)/i.test(
      text,
    );

  const genericMarket =
    /(crypto market|market overview|market today|analy[sz]e the market|overall market)/i.test(
      text,
    ) ||
    /(سوق الكريبتو|سوق العملات الرقمية|نظرة عامة على السوق|السوق اليوم|تحليل السوق|حلل السوق|تحليل السوق بالكامل|نظرة عامة على السوق بالكامل)/i.test(
      text,
    );
...
function buildPrompt({ recentMessages, userText, marketContext, attachments }) {
  const recentChat = recentMessages
    ...
    .map(
      (m) =>
        `${m.role === "user" ? "User" : "Assistant"}: ${stripMarkdown(m.content || "").slice(0, 700)}`,
    )
    .join("\n");
...
async function calculateIndicators(symbol = "BTCUSDT", interval = "1h") {
  const candles = await getBinanceOHLC(
    symbol,
    interval,
...
async function getMarketToolPack(symbol = "BTCUSDT", interval = "1h") {
  const [marketRes, ohlcRes, indicatorsRes, sentimentRes] =
  ...
  const series = ohlcRes.status === "fulfilled" ? ohlcRes.value : [];
  const indicators =
...
async function askCryptoAssistant({
  prompt,
  attachments = [],
  model = DEFAULT_MODEL,
}) {
  ...
  return (
    <ReactMarkdown
      ...
      components={{
        ...
        h2: ({ ...props }) => (
          <h2
            className="mb-3 mt-4 text-lg font-semibold text-white"
            {...props}
          />
        ),
        h3: ({ ...props }) => (
        ...
        ul: ({ ...props }) => (
          <ul
            className="mb-3 list-disc space-y-1 pl-5 text-slate-100"
            {...props}
          />
        ),
        ol: ({ ...props }) => (
          <ol
            className="mb-3 list-decimal space-y-1 pl-5 text-slate-100"
            {...props}
          />
        ),
  ...
  return (
    <div className="mt-4 overflow-hidden rounded-3xl border border-slate-700/50 bg-slate-900/70 shadow-md backdrop-blur-xl">
      ...
      <div className="grid grid-cols-2 gap-3 px-4 py-4 md:grid-cols-4 md:px-5">
        ...
        <StatCard
          label="EMA 20"
          value={formatCurrency(chart.indicators?.ema20)}
        />
        <StatCard
          label="EMA 50"
          value={formatCurrency(chart.indicators?.ema50)}
        />
        <StatCard
          label="Support"
                ...
                <div className="flex items-center justify-between px-3 pb-3">
                  ...
                  <button
                    ...
                    disabled={
                      isSending ||
                      isRecording ||
                      (!input.trim() && !composerAttachments.length)
                    }
                    className={cn(
