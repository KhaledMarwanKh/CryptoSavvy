...
function CoinDetails() {
  ...
  const [coin, setCoin] = useState(mockCoin);
  ...
  const updateSeries = (data, currentMode) => {
  ...
  const updateChart = (coin) => {
    if (
      chartHandler.isChartDataArrived &&
      (areaSeriesRef.current || candleSeriesRef.current)
    ) {
      ...
      <div className="max-w-7xl mx-auto space-y-4">
        ...
        <div className="bg-slate-900/70 border border-slate-700/50 backdrop-blur-xl shadow-md rounded-xl p-4 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div className="flex items-center gap-4">
            <img src={coin?.logo} alt="" className="w-12 h-12 rounded-full" />
            <div>
              <div className="flex items-center gap-2">
                <span className="text-lg font-semibold">
                  {coin?.symbol}/{coin?.baseSymbol}
                </span>
                <span className="text-slate-400 text-sm">#{coin?.index}</span>
              </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
            <Stat
              label={t("coinDetails.labels.change")}
              value={`${coin?.changePercent}%`}
              positive={coin.change24h >= 0}
            />
            <Stat
              label={t("coinDetails.labels.high")}
              value={`$${coin?.high24h?.toLocaleString()}`}
            />
            <Stat
              label={t("coinDetails.labels.low")}
              value={`$${coin?.low24h?.toLocaleString()}`}
            />
            <Stat
              label={t("coinDetails.labels.volume")}
              value={`$${(coin?.volume / 1e9)?.toFixed(2)}B`}
            />
            <Stat
              label={t("coinDetails.labels.market")}
              value={`$${(coin?.marketCap / 1e9)?.toFixed(2)}B`}
            />
          </div>
        ...
        <div className="bg-slate-900/70 border border-slate-700/50 backdrop-blur-xl shadow-md rounded-xl p-4 space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div
              className={`flex flex-wrap gap-2 ${i18n.language === "ar" ? "flex-row-reverse" : ""}`}
            >
              {INTERVALS.map((tf) => (
                <ToggleButton
                  key={tf}
            ...
            <button
              onClick={() => {
                if (!localStorage.userToken) {
                  toast.error(
                    i18n.language === "ar"
                      ? "انت غير مصرح لك بالدخول الى هذه الصفحة قم بتسجيل الدخول اولا"
                      : "You are not authorized to access this page ,login first and try again.",
                  );
                  return;
                }

                navigate(`/coin/analyze-chart/${coinId}`);
              }}
                  ...
                  <tbody>
                    {orderBook?.asks
                      ...
                      ?.map((row, i) => (
                        ...
                        >
                          <td className="py-1 text-center text-red-400">
                            {row?.price.toLocaleString()}
                          </td>
                          <td className="py-1 text-center text-slate-300">
                            {row?.quantity}
                          </td>
                          <td className="py-1 text-center text-slate-300">
