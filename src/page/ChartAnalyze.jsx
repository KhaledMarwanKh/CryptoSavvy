...
export default function ChartAnalyze() {
  ...
  const changeText = useMemo(() => {
    ...
    return (
      <span
        className={cn("text-sm font-semibold", cls)}
      >{`${sign}${num(v, 2)}%`}</span>
    );
  }, [ticker.changePct]);
  ...
  async function fetchTicker24h() {
    try {
      const res = await fetch(
        `${BINANCE_REST}/api/v3/ticker/24hr?symbol=${coinId}`,
      );
      if (!res.ok) return;
          ...
          <div className="flex flex-col gap-3">
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div
                  className={`flex flex-wrap ${i18n.language === "ar" ? "flex-row-reverse" : ""} gap-2`}
                >
                  {INTERVALS.map((i) => (
                    <SegButton
                      key={i}
            ...
            >
              ...
              <div className="flex items-center gap-2 overflow-x-auto lg:overflow-y-auto">
                {drawingTools.map((tool) => (
                  <button
                    className={`p-2 rounded-lg ${tool.name === activeTool ? "bg-blue-600 text-white font-semibold" : "text-slate-200 bg-slate-600"}`}
                ...
                <button
                  className={`p-2 rounded-lg bg-slate-600 text-slate-200 cursor-pointer active:bg-blue-600 `}
                  aria-label="Download as Image"
                  title={"Download" + " : " + "Download the chart as image"}
                  key={"Download"}
                  onClick={() => handleDownloadChartImage(containerRef)}
                >
              ...
              <div className="px-2 pb-2 flex items-center justify-between">
                <div className="text-slate-300 text-sm">
                  ...
                  <span className="text-slate-400">
                    {chartType === "candles" ? "Candlestick" : "Area"}
                  </span>
                </div>
                <button
                  type="button"
