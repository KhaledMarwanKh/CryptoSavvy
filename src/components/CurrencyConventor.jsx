...
const rates = {
  ...
  GBP: { rate: 0.8, symbol: "£", name: "British Pound" },
...
const CurrencyConverter = ({ cryptoList }) => {
  ...
  const currentCrypto =
    cryptoList.find((c) => c.id === selectedCryptoId) || cryptoList[0];
    ...
    <div className="bg-slate-900/70 flex-grow p-5 rounded-xl shadow-lg border border-slate-700/50 space-y-4">
      ...
      <div>
        ...
        <input
          ...
          value={cryptoAmount}
          onChange={(e) =>
            setCryptoAmount(Math.max(0, parseFloat(e.target.value)))
          }
      ...
      <div>
        <label
          htmlFor="selectCrypto"
          className="block text-xs font-medium text-slate-300 mb-1"
        >
          {t("dashboard.convertor.cryptoCurrency")}
        </label>
        <select
          id="selectCrypto"
      ...
      <div>
        <label
          ...
          className="block text-xs font-medium text-slate-300 mb-1"
        >
          {t("dashboard.convertor.fiatCurrency")}
        </label>
        <select
          id="selectFiat"
          value={selectedFiat}
      ...
      <div className="pt-3 border-t border-gray-700">
        ...
        <p className="text-xs text-gray-500 mt-1 text-center">
          {t("dashboard.convertor.currentPriceOf")} {currentCrypto?.baseSymbol}:{" "}
          {formatLargeNumbers(cryptoPriceUSD)?.toLocaleString()} $
        </p>
      </div>
    </div>
  );
